...
| **سيرفر ولا كلاينت** | كل دالة مكتوب فوقها `[server]` أو `[client]` |
| **تنبيه المكان الغلط** | تنادي دالة سيرفر في ملف كلاينت؟ يجيك خط تحتها قبل ما تشغّل |
...
## يعرف وين مكان الكود الصح

أشهر خطأ في فايف ام كله: تنادي دالة سيرفر في ملف كلاينت. الخطأ ما يظهر
إلا وقت التشغيل، وكونسول فايف ام ما يقول لك وين المشكلة.

الإكستنشن يمسكها وأنت تكتب:
...
lib.notify({ title = 'مرحبا' })        -- ✅ كلاينت، تمام
...
**وكيف يعرف إن هذا ملف كلاينت؟** يقرأ `fxmanifest.lua` حق المورد نفسه —
نفس الملف اللي يقرأه السيرفر:
...
يعني الجواب مأخوذ من **مصدر الحقيقة** لا من تخمين أسماء المجلدات. ولو
المانيفست ما ذكر الملف، يرجع للأسماء الشائعة (`client/` و `sv_` و
`*_client.lua`...). والملفات المشتركة (`shared_script`) ما تتفحص أصلاً
لأنها تشتغل على الجهتين.

الفحص يغطّي **‏٣٣٩ دالة** جهتها مقطوعة. تبي تطفيه؟

</div>

```jsonc
{ "fivemFrameworks.checkSide": false }
```
